...
const childLogger =  Logger.child({ requestId: '453' }) //new Logger('Peer');

class Peer extends EventEmitter
{
	constructor({ id, roomId,displayName, socket,role,OrgazierPeerId })
	{
		childLogger.info('constructor() [id:"%s"]', id);
		super();
		...
		this._OrgazierPeerId = OrgazierPeerId || "0";

		if(role && role == "organizer"){
			this._roles = [userRoles.PRESENTER,role ];
		}
		else if(this._OrgazierPeerId == id ){
			this._roles = [userRoles.PRESENTER,role ];
		}
		...
		this._displayName = displayName || false;
	...
	_handlePeer()
	{
		if (this.socket)
		{
			this.socket.on('disconnect', () =>
			{
				...
				childLogger.debug('"disconnect" event [id:%s]', this.id);
	...
	addRole(newRole)
	{
		if (!this._roles.includes(newRole))
		{
			...
			childLogger.info('addRole() | [newRole:"%s]"', newRole);
	...
	removeRole(oldRole)
	{
		if (this._roles.includes(oldRole))
		{
			...
			childLogger.info('removeRole() | [oldRole:"%s]"', oldRole);
	...
	get OrgazierPeerId()
	{
		return this._OrgazierPeerId;
	}

	set OrgazierPeerId(OrgazierPeerId)
	{
		this._OrgazierPeerId = OrgazierPeerId;
	}
	...
	get peerInfo()
	{
		const peerInfo =
		{
			...
			OrgazierPeerId     : this.OrgazierPeerId
		};
