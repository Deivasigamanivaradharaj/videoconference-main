...
const childLogger =  Logger.child({ requestId: '454' }) //new Logger('Room');
...
class Room extends EventEmitter
{
	 ...
	 */
	static async create({ mediasoupWorkers, roomId,accessCode })
	{
		childLogger.info('create() [roomId:"%s"]', roomId);
		...
		return new Room({ roomId, mediasoupRouters, audioLevelObserver,accessCode });
	}

	constructor({ roomId, mediasoupRouters, audioLevelObserver,accessCode })
	{
		childLogger.info('constructor() [roomId:"%s"]', roomId);
		...
		// Joining queue
		this._queue = new AwaitQueue.AwaitQueue();
	...
	verifyPeer({ id, token })
	{
		try
		{
			...
			childLogger.info('verifyPeer() [decoded:"%o"]', decoded);
	...
	{
		childLogger.info('handlePeer() [peer:"%s", roles:"%s", returning:"%s"]', peer.id, peer.roles, returning);
		...
		else
		{
			...
			this._hasAccess(peer, JOIN_ROOM) ?
				this._peerJoining(peer) :
	...
	_handleLobby()
	{
		...
		{
			childLogger.info('promotePeer() [promotedPeer:"%s"]', promotedPeer.id);
		...
		{
			childLogger.info('peerClosed() [closedPeer:"%s"]', closedPeer.id);
	...
	{
		childLogger.debug('selfDestructCountdown() started');
		...
		this._selfDestructTimeout = setTimeout(() =>
		{
			...
			if (this.checkEmpty() && this._lobby.checkEmpty())
			{
				...
				childLogger.debug('selfDestructCountdown() aborted; room is not empty!');
		}, 10000);
	}
	...
	_peerJoining(peer, returning = false)
	{
		this._queue.push(async () =>
		{
			...
			.catch((error) =>
			{
				childLogger.error('_peerJoining() [error:"%o"]', error);
			});
	}

	_handlePeer(peer)
	{
		childLogger.debug('_handlePeer() [peer:"%s"]', peer.id);
	...
	{
		childLogger.debug('_handlePeerClose() [peer:"%s"]', peer.id);
	...
	async _handleSocketRequest(peer, request, cb)
	{
		...
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				let joinedPeers = this._getJoinedPeers(peer);

				if (!this._hasPermission(peer, VIEW_ALL_PEERS))
				{
					joinedPeers =	Object.values(joinedPeers)
					.filter((pi) => pi.roles.includes("organizer"));
				}

				let peerInfos = joinedPeers
				...
				for (const otherPeer of joinedPeers)
				{
					 this._notification(
						...
						'newPeer',
						{
							...
							picture     : picture,
							roles       : peer.roles,
							OrgazierPeerId : peer.OrgazierPeerId
						}
					);
				}
			...
			case 'createWebRtcTransport':
			{
				...
				transport.on('dtlsstatechange', (dtlsState) =>
				{
					if (dtlsState === 'failed' || dtlsState === 'closed')
						childLogger.warn('WebRtcTransport "dtlsstatechange" event [dtlsState:%s]', dtlsState);
				});
			...
			case 'produce':
			{
				...
				cb(null, { id: producer.id });
				 ...
				 let consumerPeers = this._getJoinedPeers(peer)
				

				// Optimization: Create a server-side Consumer for each Peer.
				for (const otherPeer of  consumerPeers)
				{
					this._createConsumer(
						{
			...
			case 'moderator:unmute':
			{
				...
				this._notification(mutePeer.socket, 'moderator:unmute');
			...
			case 'moderator:unmuteAll':
			{
				if (!this._hasPermission(peer, MODERATE_ROOM))
					throw new Error('peer not authorized');

				// Spread to others
				this._notification(peer.socket, 'moderator:unmute', null, true);
			...
			case 'moderator:playAllVideo':
			{
				if (!this._hasPermission(peer, MODERATE_ROOM))
					throw new Error('peer not authorized');

				// Spread to others
				this._notification(peer.socket, 'moderator:playVideo', null, true);
			...
			case 'moderator:closeMeeting':
			{
				...
				this._notification(peer.socket, 'moderator:kick', null,	false);
				// Close the room
				this.close();
				cb();
			...
			default:
			{
				childLogger.error('unknown request.method "%s"', request.method);
	...
	async _createConsumer({ consumerPeer, producerPeer, producer })
	{
		...
		if (!transport)
		{
			childLogger.warn('_createConsumer() | Transport for consuming not found');
		...
		{
			childLogger.warn('_createConsumer() | [error:"%o"]', error);
	...
	{
		childLogger.debug('_request() [method:"%s", data:"%o"]', method, data);
		...
		for (let tries = 0; tries < requestRetries; tries++)
		{
			...
			catch (error)
			{
				if (
					...
					childLogger.warn('_request() | timeout, retrying [attempt:"%s"]', tries);
				else
					throw error;
			}
		}
